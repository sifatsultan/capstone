...
/**
 * This activity displays Picasa's current featured images. It uses a service
 * running a background thread to download Picasa's "featured image" RSS feed.
 * <p>
...
public class DisplayActivity extends FragmentActivity implements
	...
	public void setFullScreen(boolean fullscreen) {
		...
		if (Build.VERSION.SDK_INT >= 11) {
			...
			// Sets the View to be "low profile". Status and navigation bar
	...
	/*
	 * A callback invoked when the task's back stack changes. This allows the
	 * app to move to the previous state of the Fragment being displayed.
	 */
	@Override
	public void onBackStackChanged() {
		...
		/*
		 * If the current stack count is less than the previous, something was
		 * popped off the stack probably because the user clicked Back.
		 */
	 ...
	 * It saves the full screen status, so it can be restored when the Activity
	 * is restored
	 */
	@Override
	...
	/*
	 * This callback is invoked when the Activity is first created. It sets up
	 * the Activity's window and initializes the Fragments associated with the
	 * Activity
	 */
	@Override
	public void onCreate(Bundle stateBundle) {
		...
		/*
		 * Detects if side-by-side display should be enabled. It's only
		 * available on xlarge and sw600dp devices (for example, tablets). The
		 * setting in res/values/ is "false", but this is overridden in
		 * values-xlarge and values-sw600dp.
		 */
		mSideBySide = getResources().getBoolean(R.bool.sideBySide);

		/*
		 * Detects if hiding navigation controls should be enabled. On xlarge
		 * andsw600dp, it should be false, to avoid having the user enter an
		 * additional tap.
		 */
		mHideNavigation = getResources().getBoolean(R.bool.hideNavigation);

		/*
		 * Adds the back stack change listener defined in this Activity as the
		 * listener for the FragmentManager. See the method
		 * onBackStackChanged().
		 */
		localFragmentManager.addOnBackStackChangedListener(this);

		// If the incoming state of the Activity is null, sets the initial view
		// to be thumbnails
		if (null == stateBundle) {
	...
	/*
	 * This callback is invoked when the system is stopping the Activity. It
	...
	/**
	 * This class uses the BroadcastReceiver framework to detect and handle
	 * status messages from the service that downloads URLs.
	 */
	private class DownloadStateReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			case Constants.STATE_ACTION_COMPLETE:
				...
				PhotoThumbnailFragment localThumbnailFragment = (PhotoThumbnailFragment) getSupportFragmentManager()
						.findFragmentByTag(Constants.THUMBNAIL_FRAGMENT_TAG);

				// If the thumbnail Fragment is hidden, don't change its display
				// status
	...
	/**
	 * This class uses the broadcast receiver framework to detect incoming
	 * broadcast Intents and change the currently-visible fragment based on the
	 * Intent action. It adds or replaces Fragments as necessary, depending on
	...
	private class FragmentDisplayer extends BroadcastReceiver {
		...
		/**
		 * Receives broadcast Intents for viewing or zooming pictures, and
		 * displays the appropriate Fragment.
		 * 
		 * @param context
		 *            The current Context of the callback
		 * @param intent
		 *            The broadcast Intent that triggered the callback
		 */
		@Override
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(Constants.ACTION_VIEW_IMAGE)) {
				...
				photoFragment = (PhotoFragment) fragmentManager1
						.findFragmentByTag(Constants.PHOTO_FRAGMENT_TAG);
				...
				if (null != photoFragment) {
					...
					if (!urlString.equals(photoFragment.getURLString())) {
						...
						// Sets the Fragment to use the URL from the Intent for
						// the photo
					...
					// Sets the Fragment to use the URL from the Intent for the
					// photo
					photoFragment.setPhoto(urlString);

					// Starts a new Fragment transaction
					FragmentTransaction localFragmentTransaction2 = fragmentManager1
							.beginTransaction();

					// If the fragments are side-by-side, adds the photo
					// Fragment to the display
					if (mSideBySide) {
						localFragmentTransaction2.add(R.id.fragmentHost,
								photoFragment, Constants.PHOTO_FRAGMENT_TAG);
						/*
						 * If the Fragments are not side-by-side, replaces the
						 * current Fragment with the photo Fragment
						 */
					} else {
					...
					// Don't remember the transaction (sets the Fragment
					// backstack to null)
				...
				// If not in side-by-side mode, sets "full screen", so that no
				// controls are visible
				if (!mSideBySide)
					setFullScreen(true);

				/*
				 * If the incoming Intent is a request to zoom in on an existing
				 * image (Notice that zooming is only supported on large-screen
				 * devices)
				 */
				...
				if (mSideBySide) {
					...
					PhotoThumbnailFragment localThumbnailFragment = (PhotoThumbnailFragment) localFragmentManager2
							.findFragmentByTag(Constants.THUMBNAIL_FRAGMENT_TAG);
					...
					if (null != localThumbnailFragment) {
						...
						if (localThumbnailFragment.isVisible()) {
							...
							FragmentTransaction localFragmentTransaction2 = localFragmentManager2
									.beginTransaction();

							/*
							 * Hides the current thumbnail, clears the
							 * backstack, and commits the transaction
							 */
							localFragmentTransaction2
									.hide(localThumbnailFragment);
							localFragmentTransaction2.addToBackStack(null);
							localFragmentTransaction2.commit();

							// If the existing instance is not visible, display
							// it by going "Back"
						} else {

							// Pops the back stack to show the previous Fragment
							// state
